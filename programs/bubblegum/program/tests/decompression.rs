...
#[tokio::test]
async fn test_decompress_passes_create_v2() {
    let context = BubblegumTestContext::new().await.unwrap();

    let (mut tree, mut leaves) = context
        .create_decompressable_v2_and_mint::<MAX_DEPTH, MAX_BUF_SIZE>(DEFAULT_NUM_MINTS)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_decompress_fails_when_disabled_create_v2() {
    let context = BubblegumTestContext::new().await.unwrap();

    let (mut tree, mut leaves) = context
        .default_create_v2_and_mint::<MAX_DEPTH, MAX_BUF_SIZE>(DEFAULT_NUM_MINTS)
        .await
        .unwrap();
