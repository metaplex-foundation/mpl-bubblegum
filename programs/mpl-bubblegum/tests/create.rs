...
use mpl_bubblegum::{instruction::CreateArgs, state::MyAccount};
...
#[tokio::test]
async fn create() {
    let mut context = ProgramTest::new("mpl_bubblegum", mpl_bubblegum::ID, None)
        .start_with_context()
        .await;
    ...
    let ix = mpl_bubblegum::instruction::create(
        &address.pubkey(),
