...
import { TreeConfig, createTreeV2, fetchTreeConfigFromSeeds } from '../src';
import { createUmi } from './_setup';

test('it can create a Bubblegum tree with V2 instruction', async (t) => {
...
test('it can create a Bubblegum tree with V2 instruction using a newer size', async (t) => {
  // Given a brand new merkle tree signer.
  const umi = await createUmi();
  const merkleTree = generateSigner(umi);
